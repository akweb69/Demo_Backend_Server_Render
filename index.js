...
app.post("/referral-bonus", async (req, res) => {
  ...
  const result = await db.collection("referralBonus").insertOne(referralData);
  res.send(result);
});
...
app.post("/addrefferdiscount", async (req, res) => {
  const referralData = req.body;
  const result = await db.collection("RefferDiscount").insertOne(referralData);
  res.send(result);
});
...
app.post("/class-management", async (req, res) => {
  ...
  const result = await db.collection("classManagement").insertOne(classData);
  res.send(result);
});
...
app.post("/gift-certificate", async (req, res) => {
  const classData = req.body;
  const result = await db.collection("gift-certificate").insertOne(classData);
  res.send(result);
});
...
app.get("/single-page-data/:email", async (req, res) => {
  const email = req.params.email;
  const data = await db.collection("singlePageData").findOne({ email: email });
  res.send(data);
});
...
app.post("/landingpage-view-analytics", async (req, res) => {
  ...
  const result = await db.collection("landingPageAnalytics").insertOne(data);
  res.send(result);
});
...
app.post("/product-click-analytics", async (req, res) => {
  ...
  const result = await db.collection("productClickAnalytics").insertOne(data);
  res.send(result);
});
...
app.get("/get_data_for_my_landing_page/:subDomain", async (req, res) => {
  ...
  const user = users.find((u) => {
    const shopName = u?.storeInfo?.shopName?.toLowerCase().replace(/\s+/g, ""); // normalize DB value
...
app.post("/landingPageView", async (req, res) => {
  ...
  const result = await db.collection("landingPageView").insertOne(finalData);
  res.send(result);
});
...
app.get("/blog", async (req, res) => {
  const result = await db.collection("blog").find().sort({ _id: -1 }).toArray();
  res.send(result);
});

app.get("/blog/:id", async (req, res) => {
  const id = req.params.id;
  const result = await db.collection("blog").findOne({ _id: new ObjectId(id) });
  res.send(result);
});
